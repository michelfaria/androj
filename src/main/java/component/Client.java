...
public class Client {
	...
	public void init() {
		orderEventListeners();
		LOGGER.info(
				"Event listeners: " + eventListeners.stream().map(Object::toString).collect(Collectors.joining("\n")));
		promptIfNoToken();
		try {
			this.jda = new JDABuilder(AccountType.BOT).setToken(cfg.getToken())
					.addEventListener(eventListeners.toArray()).buildBlocking();
		} catch (Exception ex) {
	...
	public void promptIfNoToken() {
		if (env.getProperty("bot.token") == null) {
			...
			cfg.setToken(String.valueOf(System.console().readPassword()));
		}
	}
}
