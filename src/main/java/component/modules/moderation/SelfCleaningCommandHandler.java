...
public class SelfCleaningCommandHandler extends AbstractCommandHandler implements RegisteredCommandHandler {
	...
	public SelfCleaningCommandHandler(CommandHandlingFacadeBuilder builder, MessageCleaner messageCleaner,
		...
		GuildPermissionValidator permissionValidator = permissionValidatorBuilder
				.setRequiredPermissions(Collections.singletonList(Permission.MESSAGE_MANAGE)).build();
		this.facade = builder.setCmdId("clean")
				.setValidators(Arrays.asList(permissionValidator, new MaxArgsValidator(1),
						new IntArgValidator(0, IntArgValidator.Enforce.POSITIVE,
								() -> "Please specify the amount of my messages that you want to clean!")))
				.setSyntaxSupplier(() -> "[amount]").setValidatedCommandHandler(this::handle_).build();
